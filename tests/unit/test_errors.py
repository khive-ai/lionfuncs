...
@pytest.mark.parametrize(
    ...
    [
        ...
        (errors.CircuitBreakerOpenError, "Circuit breaker open", errors.LionNetworkError),
...
def test_specific_lion_errors(error_class, message_prefix, base_class):
    ...
    if error_class in [errors.APIClientError, errors.APIConnectionError, errors.APITimeoutError,
                       errors.AuthenticationError, errors.ResourceNotFoundError, errors.ServerError]:
...
def test_api_client_error_with_response_content():
    ...
    err = errors.APIClientError("Error with content", status_code=500, response_content=content)
