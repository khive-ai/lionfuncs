...
class TestExecutor:
    ...
    async def test_worker_success(self, executor):
        ...
        # Create task data
        task_data = {"api_coro": mock_api_coro, "event": event}
        ...
        assert (
            len(event.logs) >= 4
        )  # At least 4 log entries (status changes, rate limit acquisitions)
        ...
        assert any(
            "Acquired API token rate limit (50 tokens)" in log for log in event.logs
        )
        assert any(
            "Status changed from PROCESSING to CALLING" in log for log in event.logs
        )
        assert any(
            "Status changed from CALLING to COMPLETED" in log for log in event.logs
        )
    ...
    async def test_rate_limiting(self):
        ...
        try:
            ...
            for i in range(3):
                event = await executor.submit_task(
                    api_call_coroutine=mock_api_coro, num_api_tokens_needed=50
                )
                events.append(event)
    ...
    async def test_concurrency_limiting(self):
        ...
        try:
            ...
            events = []
            for i in range(5):
                event = await executor.submit_task(api_call_coroutine=delayed_api_coro)
                events.append(event)
