...
class TestIModel:
    ...
    async def test_init_with_invalid_config(self, mock_executor):
        ...
        with pytest.raises(TypeError, match="model_endpoint_config must be a dict or EndpointConfig"):
    ...
    async def test_acompletion(self, mock_executor, model_config_dict):
        ...
        assert call_args["payload"]["temperature"] == 0.5 or call_args["payload"]["temperature"] == 0.7
    ...
    async def test_context_manager(self, mock_executor, model_config_dict):
        ...
        with patch("aiohttp.ClientSession", return_value=MagicMock(spec=aiohttp.ClientSession)) as mock_session:
