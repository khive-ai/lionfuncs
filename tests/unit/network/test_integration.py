    ...
    async def test_http_flow(self):
        ...
        try:
            ...
            with patch("lionfuncs.network.endpoint.AsyncAPIClient") as mock_client_class:
                ...
                assert event.response_body[2] == {"result": "success"}  # (status_code, headers, body)
    ...
    async def test_sdk_flow(self):
        ...
        try:
            ...
            with patch("lionfuncs.network.endpoint.create_sdk_adapter") as mock_create_adapter:
                ...
                # Make a request using invoke
                event = await model.invoke(
                    request_payload={"messages": [{"role": "user", "content": "Hello, world!"}]},
                    num_api_tokens_needed=10,
                    temperature=0.7
                )
                ...
                assert call_args[1]["temperature"] == 0.7  # From additional_request_params
        finally:
            # Stop the executor
            await executor.stop()
...
# Import asyncio at the module level to avoid issues with the async tests
import asyncio