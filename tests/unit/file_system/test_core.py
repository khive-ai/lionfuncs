...
def test_create_path_timestamp(tmp_path: Path):
    ...
    assert file_path.name.endswith("_ts_file.txt") or file_path.stem.startswith("ts_file_")
...
def test_create_path_file_exists_not_ok(tmp_path: Path):
    ...
    file_path.touch()
    with pytest.raises(LionFileError, match="already exists and file_exist_ok is False"):
...
def test_chunk_content(content, chunk_by, chunk_size, overlap_ratio, threshold, expected_num_chunks, expected_first_chunk_content_approx):
    chunks = fs_core.chunk_content(content, chunk_by=chunk_by, chunk_size=chunk_size, overlap_ratio=overlap_ratio, threshold=threshold)
...
def test_chunk_content_invalid_overlap_ratio():
    with pytest.raises(LionFileError, match="Overlap ratio must be between 0.0 and <1.0"):
...
async def test_read_file_permission_error(mock_aio_open, tmp_path: Path):
    ...
    with pytest.raises(LionFileError, match=f"Permission denied when reading file: {test_file}"):
...
async def test_save_to_file_overwrite_disallowed(tmp_path: Path):
    ...
    with pytest.raises(LionFileError, match="already exists and file_exist_ok is False"):
        await fs_core.save_to_file("new content", dir_path, filename, file_exist_ok=False)
...
async def test_save_to_file_overwrite_allowed(tmp_path: Path):
    ...
    saved_path = await fs_core.save_to_file(new_content, dir_path, filename, file_exist_ok=True)
...
async def test_save_to_file_os_error_on_write(mock_aio_open, tmp_path: Path):
    ...
    with pytest.raises(LionFileError, match=f"Failed to save file {filename} in {dir_path}: Disk full"):
...
def test_list_files_simple(tmp_path: Path):
    ...
    assert sorted([p.name for p in files]) == sorted(["file1.txt", "file2.log"]) # Non-recursive
...
def test_dir_to_files_not_a_dir_error(tmp_path: Path):
    ...
    file_path.touch()
    with pytest.raises(LionFileError, match="The provided path is not a valid directory"):
...
async def test_concat_files_with_output(tmp_path: Path):
    ...
    await fs_core.concat_files(tmp_path, file_types=[".txt"], output_dir=output_dir, output_filename=output_filename)
...
async def test_concat_files_content_threshold(tmp_path: Path):
    ...
    result = await fs_core.concat_files(tmp_path, file_types=[".txt"], content_threshold=10)
