...
class Endpoint:
    ...
    def __init__(self, config: ServiceEndpointConfig):
        ...
        logger.debug(f"Initialized Endpoint with name={self.config.name}, "
    ...
    async def _create_client(self) -> Union[AsyncAPIClient, AbstractSDKAdapter]:
        ...
        if self.config.transport_type == "http":
            ...
            logger.debug(f"Creating SDK adapter for endpoint {self.config.name} "
            ...
            async with self._lock:
                if self._client_instance is None:  # Double-check after acquiring lock
                    ...
                    if hasattr(self._client_instance, '__aenter__'):
                        ...
                        logger.debug(f"Entered async context for client in endpoint {self.config.name}")
        ...
        async with self._lock:
            if self._client_instance:
                ...
                if hasattr(client_to_close, 'close'):
                    ...
                    if asyncio.iscoroutinefunction(close_method) or isinstance(close_method, AsyncMock):
                        await close_method()
                    else:
                ...
                elif hasattr(client_to_close, '__aexit__') and client_to_close.__aexit__ is not None:
