...
def _extract_docstring_parts(docstring: str | None) -> tuple[str, Dict[str, str]]:
    ...
    args_section_match = re.search(r"(Args|Parameters):\s*\n((?:.|\n)*?)(?=\n\n|\Z)", docstring, re.MULTILINE)
    ...
    if args_section_match:
        ...
        param_matches = re.finditer(r"^\s*([_a-zA-Z]\w*)\s*\(.*?\)?\s*:\s*(.*?)(?=\n\s*[_a-zA-Z]\w*\s*\(|\Z)", args_content, re.MULTILINE | re.DOTALL)
        if not list(re.finditer(r"^\s*([_a-zA-Z]\w*)\s*\(.*?\)?\s*:\s*(.*?)(?=\n\s*[_a-zA-Z]\w*\s*\(|\Z)", args_content, re.MULTILINE | re.DOTALL)): # check if any matches found
            # Fallback for simpler "param: description" format if the above doesn't match
            param_matches = re.finditer(r"^\s*([_a-zA-Z]\w*)\s*:\s*(.*?)(?=\n\s*[_a-zA-Z]\w*\s*:|\Z)", args_content, re.MULTILINE | re.DOTALL)
        ...
        for match in param_matches:
            ...
            param_descriptions[param_name] = " ".join(line.strip() for line in description_lines)
